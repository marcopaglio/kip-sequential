...
std::unique_ptr<Kernel> createKernel(const std::string& name, unsigned int order, const std::vector<int> &weights,
        const int normFactor) {
    return std::make_unique<Kernel>(name, order, weights, normFactor);
}
...
std::unique_ptr<Kernel> KernelFactory::createBoxBlurKernel(const unsigned int order) {
    ...
    return createKernel("boxBlur", order, std::vector(num_of_values, 1),
        static_cast<int>(num_of_values));
}

std::unique_ptr<Kernel> KernelFactory::createEdgeDetectionKernel(const unsigned int order) {
    checkOrderValidity(order);

    std::vector<int> weights(order * order, -1);
    ...
    for (unsigned int k = 1; k < corePoint; k++) {
        ...
        for (unsigned int j = k; j < order - k; j++) {
            for (unsigned int i = k; i < order - k; i++) {
                weights[j * order + i] -= static_cast<int>(difference);
            }
        }
        coreWeight += difference * ((order - 2*k)*(order - 2*k) - 1);
    }
    weights[corePoint * order + corePoint] = static_cast<int>(coreWeight);

    return createKernel("edgeDetection", order, weights, 1);
}